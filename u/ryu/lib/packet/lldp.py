...
@lldp.set_tlv_type(LLDP_TLV_TOPOLOGY_INFORMATION)
class Topology(LLDPBasicTLV):
    _TOPO_LENGTH = 1
    _PACK_STR = '!H'
    _PACK_SIZE = struct.calcsize(_PACK_STR)

    def __init__(self, buf=None, *args, **kwargs):
        super(Topology, self).__init__(buf, *args, **kwargs)
        if buf:
            pass
        else:
            self.topology = kwargs["topology"]
            self._TOPO_LENGTH = len(self.topology)
            # self.len = self._PACK_SIZE + len(self.topology)
            # self.len = self._PACK_SIZE
            self.len = struct.calcsize('!%ds'%self._TOPO_LENGTH)
            assert self._len_valid()
            self.typelen = (self.tlv_type << LLDP_TLV_TYPE_SHIFT) | self.len

    def serialize(self):
        return struct.pack('!H%ds'%self._TOPO_LENGTH, self.typelen, self.topology)
