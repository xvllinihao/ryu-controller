...
class LLDPPacket(object):
    ...
    @staticmethod
    def lldp_packet(dpid, port_no, dl_addr, ttl, topology=None):
        pkt = packet.Packet()
        ...
        if topology:
            tlv_topology = lldp.Topology(topology=topology)
            tlv_end = lldp.End()
            tlvs = (tlv_chassis_id, tlv_port_id, tlv_ttl, tlv_topology, tlv_end)
        else:
            tlv_end = lldp.End()
    ...
    def lldp_parse(data):
        ...
        if len(lldp_pkt.tlvs)==5:
            tlv_topo = lldp_pkt.tlvs[3]
            return  src_dpid, src_port_no, tlv_topo
...
class Switches(app_manager.RyuApp):
    ...
    def lldp_packet_in_handler(self, ev):
        ...
        msg = ev.msg
        try:
            infos = LLDPPacket.lldp_parse(msg.data)
